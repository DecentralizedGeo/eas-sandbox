...
/**
 * Runs the ProofMode workflow with an uploaded file:
 * 1. Ensures the ProofMode schema is registered
 * 2. Extracts the uploaded ProofMode zip file
 ...
 * 6. Returns the attestation data
 * 
 * @param zipFilePath - Path to the uploaded zip file
 * @returns The attestation data including UID, location, and timestamp
 */
export async function runProofModeWorkflowWithFile(zipFilePath: string): Promise<{
    ...
    try {
        ...
        // 2. Extract the uploaded ProofMode zip file
        console.log(`\nStep 2: Processing uploaded zip file: ${zipFilePath}`);
        
        // Create a directory to extract the zip file to (use the same name as the zip file without the .zip extension)
        const extractDir = `${zipFilePath.replace('.zip', '')}-extracted`;

        // Extract the zip file
        extractZipFile(zipFilePath, extractDir);

        if (!fs.existsSync(extractDir)) {
            throw new Error(`Extraction folder not found at path: ${extractDir}`);
        }
